...
{
    if (category_name.length() == 0) { std::cout << "Empty strings are not allowed for category names!\n"; exit(1); }
...
void ParameterHandler::DefineStringParameter(const std::string &parameter_name, const std::string &short_description, const std::string &default_value, const std::string& category_name, const std::vector<std::string> &allowed_values)
{
    ...
    if (parameter_name.length() == 0) { std::cout << "Empty strings are not allowed for parameter names!\n"; exit(1); }
